...
class cSpecification extends DBManager {

	private $DATA_SAVE_PATH;				// 저장경로.
	private $ALLOW_FILE_EXT;				// 업로드 가능 확장자

	function cSpecification($DATA_SAVE_PATH,$ALLOW_FILE_EXT) {
    ...
    function getList($ARR){
        ...
        // 처리여부
        if($ARR['NSU_PROC_YN_SEARCH'] != '') {
			$tmpKeyword .= " AND INSTR(`NSU`.`NSU_PROC_YN`,'".addslashes($ARR['NSU_PROC_YN_SEARCH'])."')>0";
		}
...
// 0. 객체 생성
$cSpecification = new cSpecification(FILE_UPLOAD_PATH,ALLOW_FILE_EXT); //서버용
$cSpecification->ConnectDB($dbHOST, $dbUSER, $dbPASS, $dbNAME, $dbPORT, $sslPath);
