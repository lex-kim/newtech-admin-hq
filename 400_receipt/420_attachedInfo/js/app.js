const ATTACH_URL = 'act/attachedInfo_act.php';
let CURR_PAGE = 1; 
let tdLength = 0;
...
getList = (currentPage) =>{
    ...
    callAjax(ATTACH_URL, 'GET', param).then((result)=>{
          ...
          $('#indicator').hide();
            ...
            item.map((data, i)=>{
              ...
              if($("#AUTH_UPDATE").val() == YN_Y){
                tableTxt += "<td><button type='button' class='btn btn-default' onclick='openWindowPopup(\""+data.MGG_ID+"\",\""+data.NSU_FILE_IMG+"\")'>수정</button></td>";
              }
              tableTxt += "</tr>";
            //   getStaticTable(data);
            })
...
openWindowPopup = (id, img) => {
  console.log('id >',id,'img>',img);
    const formData = $('#attachedForm');
    window.open('', 
              'newwindow', 
...
imgZoomPopup = () => {
    ...
    const formData = $('#attachedImgForm');
    window.open('', 
    'newwindowImg', 
...
/* 엑셀다운로드 */
excelDownload = () => {
    location.href="attachedInfoExcel.html?"+$("#searchForm").serialize();
}


proc = (id, nm) => {
  confirmSwalTwoButton(nm+'공업사의 첨부정보 처리를 완료 하시겠습니까?','확인', '취소', false ,()=>{
    const param ={
      REQ_MODE: CASE_UPDATE,
      NSU_ID: id
    }
    callAjax(ATTACH_URL, 'POST', param).then((result)=>{
