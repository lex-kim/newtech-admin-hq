	...
	class InOutOrderController extends Options{
		...
		function detailIndex(){
			...
			if($result['status'] == OK){
				...
				errorAlert("발주 정보를 가져오는데 실패하였습니다.");
			}
		...
		function popupIndex(){
			...
			if($partnerListResult['status'] == OK){
				...
				errorAlert("매입처정보를 가져오는데 실패하였습니다.");
			}
			...
			if(!empty($_REQUEST['orderID'])){
				...
				if($orderResult['status'] == OK){
					...
					errorAlert("원주문 정보를 가져오는데 실패하였습니다.");
				...
				$shipZipCode = $companyInfo['stockZipCode'];
				$shipAddr1 = $companyInfo['stockAddress1'];
				$shipAddr2 = $companyInfo['stockAddress2'];
