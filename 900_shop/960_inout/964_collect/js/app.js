...
function setAutocomplete(dataArray, objID, type){
  try{
    ...
    $("#"+objID).autocomplete({
        ...
        source: data.map((data, index) => {
            return {
                id : data[type+'ID'],
                value : data[type+'Name']+(!empty(data.rankName) ? "("+data.rankName+")" : "")
            };
        }),
    ...
    alert("정보를 가져오는데 실패하였습니다. 관라자에게 문의해주세요.")
  }  
}
...
/**
 * 동적으로 추가될 행에서 처리구분(분실/폐기) select를 동적으로 생성할 Html 리턴
 * @param{몇번째 선택된 행인지}idx
 */
getLostIdSelectHtml = (idx) => {
  let selectHtml = "<select name='lostDisposalID[]' class='form-control'  id='lostDisposalID_"+idx+"' required>";
  selectHtml += "<option value=''>=선택=</option>";
  lostCodeDomainInfo.map((item)=>{
    selectHtml += "<option value='"+item.code+"'>"+item.domain+"</option>"
  });
  selectHtml += "</select>";
...
getStockInputTableHtml = () => {
  ...
  html += '</td>';
  html += '<td class="text-left">';
  html += "<input type='text' class='form-control text-right' id='unitPrice_"+stockTableTrIdx+"'  onkeyup='isNumber(event)' name='unitPrice[]' readonly/>";
  html += '</td>';
  html += '<td class="text-left">';
...
setTable = (dataList, totalListCount, cPage) => {
  ...
  dataList.map((data)=>{
      ...
      html += "<td class='text-left'>"+data.divisionName+"</td>";
      html += "<td class='text-center'>"+data.regionName+"</td>";
      html += "<td class='text-left'>"+data.garageName+"</td>";
      html += "<td class='text-left'>"+data.contractStatusName+"</td>";
      html += "<td class='text-left'>"+data.categoryName+"</td>";
      ...
      html += "<td class='text-right'>"+(empty(data.lastDatetime) ? "-" : data.lastDatetime)+"</td>";
      html += "<td class='text-left'>"+data.memo+"</td>";
      html += "<td class='text-left'>"+data.issueUserName+"</td>";
...
excelDownload = () => {
  location.href="collectExcel.php?"+$("#searchForm").serialize()+"&REQ_MODE="+CASE_TOTAL_LIST;
}
...
setData = (REQ_MODE) => {
    ...
    callFormAjax(ERP_CONTROLLER_URL, 'POST', formData).then((result)=>{
      if(isJson(result)){
        const res = JSON.parse(result);
        if(res.status == OK){
          alert("회수 등록이 완료되었습니다.");

          window.close();
          window.opener.getList(1);
        }else{
          if(res.data == NOT_ENOUGH_QUANTITIES){
            alert("잔여재고가 부족합니다. 잔여재고를 확인해주세요.");
          }else{
            alert(ERROR_MSG_1100);
          }
        }
...
/* 등록 모달창*/ 
showAddModal = () => {
  const url = '/900_shop/960_inout/964_collect/collectModal.php';
  openWindowPopup(url, 'releaseModal', 1000, 400);
}
