...
<div class="modal-add" id="add-new">
    <form class="form-horizontal" name="editForm" id="editForm" method="post">
        ...
        <div class="modal-body">   
            ...
            <div class="row">
                <!-- 회수일 -->
                <div class="col-md-1"><label for="issueDate">회수일</label></div>
                <div class="col-md-3">
                    <input type="text" class="form-control" id="issueDate" name="issueDate" autocomplete="off" placeholder="현재일 기본세팅-변경가능">
                    <span class="glyphicon glyphicon-calendar search-glypicon"></span>
                </div>
                <!-- 직원명 -->
                <div class="col-md-1"><label for="SALESMAN_NM">직원명</label></div>
                <div class="col-md-3">
                    ...
                    <?php
                         echo "<script>setAutocomplete('".$userInfo."', 'issueUser', 'user');</script>";
                    ?>
                </div>
                <!-- 공업사 -->
                <div class="col-md-1"><label for="garageID">공업사</label></div>
                <div class="col-md-3">
                    <input type="text" required  class="form-control autocomplete" id="garageID" placeholder="입력 후 선택"/>
                    <input type="hidden"  name="garageID" id="garageIDHidden"/>
                    <?php
                         echo "<script>setAutocomplete('".$garageInfo."', 'garageID', 'garage');</script>";
                    ?>
                </div>
            </div>      
            ...
            <!-- 메모 -->
            <div class="row">
                <div class="col-md-1"><label for="COLLECT_MEMO">메모</label></div>
                <div class="col-md-7"><input type="text" class="form-control" id="COLLECT_MEMO" name="COLLECT_MEMO" placeholder="입력"></div>
            </div>   
...
<script>
    ...
    $('#editForm').submit(function(e){
        ...
        if(empty($('#issueUserHidden').val())){
            ...
            $('#issueUser').focus();
        }else if(empty($('#garageIDHidden').val())){
            alert("공업사를 입력 후 선택해주셔야 합니다.");
            $('#garageID').focus();
        }else{
