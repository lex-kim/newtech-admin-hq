...
function setUserAutocomplete(userInfoArr){
    try{
      const userInfo = JSON.parse(userInfoArr);
      $("#issueUser").autocomplete({
          minLength : 1,
          source: userInfo.map((data, index) => {
              return {
                  id : data.userID,
                  value : data.userName +"("+data.rankName+")"
              };
          }),
          ...
          select: function(event, ui) {
              ...
              $("#issueUser").val(strArea);
              $('#issueUserHidden').val(ui.item.id);
              return false;
          }
      }).on('focus', function(){ 
...
setTable = (dataList, totalListCount, cPage) => {
  ...
  dataList.map((data)=>{
      ...
      html += "<td class='text-left'>"+data.lostDisposal+"</td>";
...
excelDownload = () => {
  location.href="lostExcel.php?"+$("#searchForm").serialize()+"&REQ_MODE="+CASE_TOTAL_LIST;
}
...
/* 등록 모달창*/ 
showAddModal = () => {
  const url = '/900_shop/960_inout/963_lost/lostModal.php';
