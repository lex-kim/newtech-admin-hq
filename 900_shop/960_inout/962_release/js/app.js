...
setTable = (dataList, totalListCount, cPage) => {
  ...
  dataList.map((data)=>{
      ...
      html += "<td class='text-center'>"+(empty(data.lastDatetime) ? "-" : data.lastDatetime)+"</td>";
      html += "<td class='text-left'>"+data.memo+"</td>";
      html += "<td class='text-center'>"+(empty(data.issueUserName) ? "-" : data.issueUserName)+"</td>";
...
excelDownload = () => {
  location.href="releaseExcel.php?"+$("#searchForm").serialize()+"&REQ_MODE="+CASE_TOTAL_LIST;
}
...
/* 등록 모달창*/ 
showAddModal = () => {
  const url = '/900_shop/960_inout/962_release/releaseModal.php';
