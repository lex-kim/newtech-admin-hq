...
<div class="container-fluid mb20px">
  <h3 class="nwt-category"><small>입출고 현황&nbsp;&nbsp;></small>직원별 수불현황</h3>
</div>


<?php if(empty($cUserArray)):?>
    <div class="container-fluid nwt-search-form" style="text-align:center; margin-top:120px">
        <h2>등록된 직원 데이터가 존재하지 않습니다.</h2>
        <span style="font-size:13px; color:gray">직원 등록 후 이용 가능한 페이지 입니다.</span>
    </div>
  </body>
  </html>
...
<?php else:?>
                      ...
                      <div class="frm-wrap">
                        ...
                        <div class="row frm-last-row">
                          ...
                          <div class="col-md-2">
                            ...
                            <input type="text" class="form-control" onchange="onchangeItemID()"
                              name="selectItemID" id="selectItemID" placeholder="상품ID 입력 및 선택(자동완성)">                      
    ...
    <table class="table table-bordered table-striped">
      <thead>
        <tr class="active">
          ...
          <?php 
            foreach($cUserArray as $index => $item){
          ?>
            <th class="text-center" id="<?=$item['employeeID']?>" colspan="10"><?=$item['employeeName']?></th>
          <?php
            }
      ...
      <tr id="indicatorTable" style="display : none">   
        <td colspan="<?=count($cUserArray)*10+6?>"> 
  ...
  <script>
    ...
    let userArray = [];
    try{
      userArray = JSON.parse('<?=json_encode($cUserArray, true)?>');        // 동적으로 가져온 직원 리스트
