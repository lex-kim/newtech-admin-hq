<?php
	...
	switch( $_REQUEST['REQ_MODE'] ){
		...
		/** 생성 */
		case CASE_CREATE:
			$result = $clsStandardComponent->setData($_REQUEST);
			if($result === CONFLICT){
		...
		/** 해당데이터 삭제 */
		case CASE_DELETE:
			if($data =  $clsStandardComponent->delData($_REQUEST)) {
				$response['status']  = OK;
				$response['message'] = "OK";
			} else {
				$response['status']  = SERVER_ERROR;
		...
		/** 모달에서 권한 코드 받기 */
		case GET_PARTS_TYPE:
			$data =  $clsStandardComponent->getPartsType($_REQUEST);
