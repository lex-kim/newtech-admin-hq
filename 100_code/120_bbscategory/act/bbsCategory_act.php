<?php
	...
	require "../class/clsBbsCategory.php";

	switch( $_REQUEST['REQ_MODE'] ){

		/** 생성 */
		case CASE_CREATE:
			$result = $cBbsCategory->setData($_REQUEST);
			if($result === CONFLICT){
		...
		case CASE_LIST:
			...
			$data =  $cBbsCategory->getList($_REQUEST);
		...
		/** 클릭한 해당 데이터 gET */
		case CASE_READ:
			$data =  $cBbsCategory->getData($_REQUEST);
		...
		/** 해당데이터 삭제 */
		case CASE_DELETE:
			if($data =  $cBbsCategory->delData($_REQUEST)) {
				$response['status']  = OK;
				$response['message'] = "OK";
			} else {
				$response['status']  = SERVER_ERROR;
