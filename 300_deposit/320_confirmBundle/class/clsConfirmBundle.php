...
class cBundle extends DBManager {
	...
	function getDetailList($ARR) {
		...
		$iSQL = "SELECT 
					...
					(SELECT GROUP_CONCAT(NB.`NB_ID` , 
						...
						WHERE NB.`NB_ID` = NBI.`NB_ID`
						AND NB.DEL_YN = 'N' 
						-- AND RIGHT( NBD.NHD_INSU_NM, 5) = RIGHT( NBI.`NBI_REGI_NUM`, 5) \n
	...
	function setData($ARR,$PDS) {
		...
		if($iSQL  != ERROR_DB_INSERT_DATA){
			...
			echo "[".date("Y-m-d H:i:s")."] 업로드 데이터 오류<br/>";
			return ERROR_DB_INSERT_DATA;
		}
    ...
    function getFileQuery($ARR,$filename) {
        ...
        while(!feof($fp)){
			...
			$data = explode(",",$row);
			if(count($data) == 5){
				echo $preSQL."('".$DTHMS_ID."','".$data[0]."', '".$data[1]."','".$data[2]."','".$data[3]."','".$data[4]."',NOW(),NOW(),'".addslashes($_SESSION['USERID'])."'); \n";
				...
				$vistDay= preg_replace("/\s+/", "", $data[43]);
				echo $data[3]." : ".$data[4];
				return ERROR_DB_INSERT_DATA;
			}
		}
