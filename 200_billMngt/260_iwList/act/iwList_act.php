<?php
	...
	switch( $_REQUEST['REQ_MODE'] ){
		...
		/** 생성 */
		case CASE_CREATE:
			$result = $clsIwList->setData($_REQUEST);
			if($result) {
				$response['status']  = OK;
				$response['message'] = "OK";
			}else {
		...
		case CASE_UPDATE:
			$data =  $clsIwList->setData($_REQUEST);
		...
		/** 해당년도 */
		case CASE_LIST_SUB:
			$data =  $clsIwList->getClaimYearOption($_REQUEST);
			if($data === HAVE_NO_DATA){
				$response['status']  = SERVER_ERROR;
				$response['message'] = "Internal Server Error";
			}else{
		...
		/** 조회 */
		case CASE_READ:
			$data =  $clsIwList->getData($_REQUEST);
