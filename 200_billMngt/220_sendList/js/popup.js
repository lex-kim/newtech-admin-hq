...
validation = () => {
    if(!$('#NBIT_TO_MII_ID').val()){
        alert("보험사명을 선택해주세요.");
        $('#NBIT_TO_MII_ID').focus();
        return false;
    }else if(!$('#NBIT_TO_NIC_ID').val()){
        alert("담당자를 선택해주세요.");
...
autoCompleteInsuPopup= (crrArray, objId) => {
    ...
    $("#"+objId).autocomplete({
        ...
        select: function(event, ui) {
            ...
            $('#NBIT_TO_NIC_NM').blur(); 
            $('#NBIT_TO_NIC_NM').val(ui.item.name); 
...
function transSendFax(){
    let form = $("#transSendFax")[0];
    let formData = new FormData(form);
    let sendResultCount = $('input[name="TRANS_NB_ID[]"').length;
    let sendResultSuccess = 0;
    let sendResultFail = 0;

    $('input[name="TRANS_NB_ID[]"').each(function(index){
        formData.append('NB_ID', $('#TRANS_NB_ID_'+index).val());
        formData.append('MGG_ID', $('#TRANS_MGG_ID_'+index).val());
        formData.append('NBI_FAX_RMT_ID', $('#TRANS_NBI_RMT_ID_'+index).val()); 
        formData.append('CAR_NUM', $('#TRANS_CAR_NUM_'+index).val());

        formData.append('NBI_CHG_NM', $('#NBIT_TO_NIC_NM_AES').val());
        formData.append('NBI_FAX_NUM', $('#NBIT_TO_NIC_FAX_AES').val());
        formData.append('MII_NM', $('#NBIT_TO_MII_ID option:selected').text());
        callFormAjax('../../../sendFAX.php', 'POST', formData).then((result)=>{
            ...
            if(sendResultCount == (sendResultSuccess+sendResultFail)){
                if(sendResultCount == sendResultSuccess){
                    confirmSwalOneButton('이첩에 성공하였습니다.', OK_MSG ,()=>{
                        window.opener.getList();
                        self.close();
                    });
                }else{
                    const msg = "성공 : "+sendResultSuccess+"건 , 실패 : "+sendResultFail+"건 입니다.";
                    confirmSwalOneButton("이첩엔 성공했으나 팩스발송에 실패하였습니다.", OK_MSG,()=>{
                        window.opener.getList();
                        self.close();
                   }, msg);
                }
