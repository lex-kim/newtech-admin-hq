...
class clsWriteStatus extends DBManager {
	...
	function getList($ARR) {
		...
		$NB_RATIO_PRICE = "IF(NB.RLT_ID = '11440',NB_TOTAL_PRICE*(SELECT NBI_RATIO/100 FROM NT_BILL.NBB_BILL_INSU WHERE NB_ID = NB.NB_ID)
		  					, NB_TOTAL_PRICE)" ;
		$iSQL = "SELECT 
					$DATE_FORMAT AS NB_CLAIM_DT,
					
					COUNT(DISTINCT NB.MGG_ID) AS MGG_CNT,
					COUNT(DISTINCT IF(NB.MODE_TARGET = '10101', NB.MGG_ID, NULL))AS MGG_11110,
					COUNT(DISTINCT IF(NB.MODE_TARGET = '10102', NB.MGG_ID, NULL))AS MGG_11111,
					COUNT(DISTINCT IF(NB.MODE_TARGET = '10103', NB.MGG_ID, NULL))AS MGG_11112,
					...
					TRUNCATE(SUM($NB_RATIO_PRICE),-1)AS NB_TOTAL_PRICE,
					FORMAT(TRUNCATE(SUM(IF(NB.MODE_TARGET = '10101' , $NB_RATIO_PRICE ,0)),-1),0) AS NB_TOTAL_PRICE_11110,
					FORMAT(TRUNCATE(SUM(IF(NB.MODE_TARGET = '10102' , $NB_RATIO_PRICE ,0)),-1),0) AS NB_TOTAL_PRICE_11111,
					FORMAT(TRUNCATE(SUM(IF(NB.MODE_TARGET = '10103' , $NB_RATIO_PRICE ,0)),-1),0) AS NB_TOTAL_PRICE_11112,
					...
					FORMAT(TRUNCATE(IFNULL((SUM(IF(NB.MODE_TARGET = '10101' , $NB_RATIO_PRICE ,0))/SUM(IF(NB.MODE_TARGET = '10101' , 1 ,0))),0),-1),0) AS NB_PRICE_11110,
					FORMAT(TRUNCATE(IFNULL((SUM(IF(NB.MODE_TARGET = '10102' , $NB_RATIO_PRICE ,0))/SUM(IF(NB.MODE_TARGET = '10102' , 1 ,0))),0),-1),0) AS NB_PRICE_11111,
					FORMAT(TRUNCATE(IFNULL((SUM(IF(NB.MODE_TARGET = '10103' , $NB_RATIO_PRICE ,0))/SUM(IF(NB.MODE_TARGET = '10103' , 1 ,0))),0),-1),0) AS NB_PRICE_11112,
				
					-- ROUND(COUNT(DISTINCT NB.MGG_ID, CASE WHEN NB.MODE_TARGET = '10101' THEN NB.RMT_ID END) /COUNT(DISTINCT NB.MGG_ID),1) *100 AS MGG_PER_11110,
					ROUND(COUNT(DISTINCT IF(NB.MODE_TARGET = '10101', NB.MGG_ID, NULL)) /COUNT(DISTINCT NB.MGG_ID)*100,1)  AS MGG_PER_11110,
					ROUND(COUNT(DISTINCT IF(NB.MODE_TARGET = '10102', NB.MGG_ID, NULL)) /COUNT(DISTINCT NB.MGG_ID)*100,1)  AS MGG_PER_11111,
					ROUND(COUNT(DISTINCT IF(NB.MODE_TARGET = '10103', NB.MGG_ID, NULL)) /COUNT(DISTINCT NB.MGG_ID)*100,1)  AS MGG_PER_11112,
		...
		$totalSQL = "SELECT 
					...
					FROM (
						SELECT 	
							...
							COUNT(DISTINCT NB.MGG_ID) AS MGG_CNT,
							COUNT(DISTINCT IF(MODE_TARGET = '10101', NB.MGG_ID, NULL))AS MGG_11110,
							COUNT(DISTINCT IF(MODE_TARGET = '10102', NB.MGG_ID, NULL))AS MGG_11111,
							COUNT(DISTINCT IF(MODE_TARGET = '10103', NB.MGG_ID, NULL))AS MGG_11112,
							...
							TRUNCATE(SUM($NB_RATIO_PRICE),-1)AS NB_TOTAL_PRICE,
							TRUNCATE(SUM(IF(NB.MODE_TARGET = '10101' , $NB_RATIO_PRICE ,0)),-1) AS NB_TOTAL_PRICE_11110,
							TRUNCATE(SUM(IF(NB.MODE_TARGET = '10102' , $NB_RATIO_PRICE ,0)),-1) AS NB_TOTAL_PRICE_11111,
							TRUNCATE(SUM(IF(NB.MODE_TARGET = '10103' , $NB_RATIO_PRICE ,0)),-1) AS NB_TOTAL_PRICE_11112,
							...
							TRUNCATE(IFNULL((SUM(IF(NB.MODE_TARGET = '10101' , $NB_RATIO_PRICE ,0))/SUM(IF(NB.MODE_TARGET = '10101' , 1 ,0))),0),-1) AS NB_PRICE_11110,
							TRUNCATE(IFNULL((SUM(IF(NB.MODE_TARGET = '10102' , $NB_RATIO_PRICE ,0))/SUM(IF(NB.MODE_TARGET = '10102' , 1 ,0))),0),-1) AS NB_PRICE_11111,
							TRUNCATE(IFNULL((SUM(IF(NB.MODE_TARGET = '10103' , $NB_RATIO_PRICE ,0))/SUM(IF(NB.MODE_TARGET = '10103' , 1 ,0))),0),-1) AS NB_PRICE_11112,
						
							-- ROUND(COUNT(DISTINCT NB.MGG_ID, CASE WHEN MODE_TARGET = '10101' THEN RMT_ID END) /COUNT(DISTINCT NB.MGG_ID),1) *100 AS MGG_PER_11110,
							ROUND(COUNT(DISTINCT IF(MODE_TARGET = '10101', NB.MGG_ID, NULL)) /COUNT(DISTINCT NB.MGG_ID)*100,1)  AS MGG_PER_11110,
							ROUND(COUNT(DISTINCT IF(MODE_TARGET = '10102', NB.MGG_ID, NULL)) /COUNT(DISTINCT NB.MGG_ID)*100,1)  AS MGG_PER_11111,
							ROUND(COUNT(DISTINCT IF(MODE_TARGET = '10103', NB.MGG_ID, NULL)) /COUNT(DISTINCT NB.MGG_ID)*100,1)  AS MGG_PER_11112,
